...
class LevelError(BaseError):
    def __init__(self, message, invalid_access_level):
        super().__init__(message)
        self.invalid_access_level = invalid_access_level
...
try:
    ...
    project.login(name, access_level)
except AccessError as ae:
    print(ae)

try:
    name = input("Введите имя нового пользователя: ")
    identifier = input("Введите личный идентификатор нового пользователя: ")
    access_level = int(input("Введите уровень доступа нового пользователя (от 1 до 7): "))
    project.add_user(name, identifier, access_level)
